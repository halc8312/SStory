...
class H19ProofTest(unittest.TestCase):
    def test_crop_proof_is_bounded_unique_and_exact(self) -> None:
        h19.DEFAULT_OUTPUT.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(
            prefix=".candidate-h19-test-", dir=h19.DEFAULT_OUTPUT
        ) as temporary:
            proof_dir = Path(temporary)
            report = h19.render_proof(output_dir=proof_dir)
            self.assertFalse(report["scope"]["full_master_written"])
            self.assertEqual(
                report["exact_protection"]["protected_violation_pixels"], 0
            )
            self.assertLess(
                report["exact_protection"][
                    "allowed_edit_coverage_percent_full_canvas"
                ],
                20.0,
            )
            self.assertEqual(
            ...
            self.assertTrue(
                report["self_vision_review"]["immediate_failure_detected"]
            )
            for name in (
                h19.PROOF_SOURCE,
                h19.PROOF_REPAIRED,
