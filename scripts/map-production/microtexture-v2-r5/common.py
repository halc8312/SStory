"""Shared r5 trust root, operational blind, one-shot loaders, and Git preflight."""
...
SPEC_SHA256 = "0f8769bad73c1de70bc46061776a9443b9459990b7fa58a322b18f0d6679fff1"
...
def validate_contact_sheet_view_partition(
    ...
    context = "r5 contact-sheet views"
    if (
...
def validate_preregistered_spec(value: dict[str, Any]) -> None:
    ...
    if not isinstance(selection, dict):
        raise RuntimeError("r5 threshold_selection must be an object")
    ...
    if (
        ...
        raise RuntimeError("r5 single hard-gate contract drift")
    ...
    if not isinstance(contact, dict) or not isinstance(contact.get("views"), list):
        raise RuntimeError("r5 contact-sheet spec must define views")
    ...
    if label_paths != {
        ...
        raise RuntimeError("r5 exact reviewed-label artifact paths drift")
    ...
    if sealed_label_paths != {
        ...
        raise RuntimeError("r5 sealed reviewed-label authority paths drift")
    ...
    if (
        ...
        or corpus["rejected_candidates_forbidden"] != ["v8", "v9"]
    ):
        raise RuntimeError("r5 foundation geometry/rejection contract drift")
    foundations = corpus["foundations"]
    if (
        not isinstance(foundations, list)
        or [item.get("id") for item in foundations] != ["v10", "v11", "v12"]
        ...
        raise RuntimeError("r5 foundation source binding drift")
    ...
    if (
        ...
        raise RuntimeError("r5 foundation independent review binding drift")
    definition = value.get("metric_definition")
    if not isinstance(definition, dict):
        raise RuntimeError("r5 metric definition must be an object")
    references = definition.get("score_reference_constants")
    require_exact_keys(references, REFERENCE_KEYS, "r5 score references")
    for key in REFERENCE_KEYS:
        require_exact_real(references[key], f"r5 score reference {key}", minimum=1e-12)
    if (
        ...
        raise RuntimeError("r5 hard composite definition drift")
    ...
    if (
        ...
        raise RuntimeError("r5 frozen metric parameter drift")
    one_shot = value.get("one_shot_failure_reporting", {})
    if (
        one_shot.get("completion_report_schema")
        != "microtexture-v2-r5-stage-completion/2"
        ...
        raise RuntimeError("r5 final stage-completion contract drift")
    if (
        ...
        raise RuntimeError("r5 terminal authority reload contract drift")
    ...
    for field, expected in (
        ("expected_unique_clusters_per_split", 78),
        ...
        ("expected_artifact_clusters_per_family", 12),
    ):
    ...
    if (
        not isinstance(labels_contract.get("public_pre_marker_validation"), str)
        or not labels_contract["public_pre_marker_validation"]
        ...
        raise RuntimeError("r5 private Vision audit contract drift")

    catalog = value.get("control_catalog_authority")
    if not isinstance(catalog, dict):
        raise RuntimeError("r5 control catalog authority must be an object")
    require_exact_keys(
        catalog,
        {
        ...
        "r5 control catalog authority",
    )
    if require_exact_int(
        catalog.get("records_per_split"), "catalog records", 1
    ) != 140 or catalog.get("private_role_records_per_split") != {
        "artifact": 120,
        "protocol-zero": 16,
        "duplicate-audit": 4,
    }:
        raise RuntimeError("r5 control catalog cardinality drift")
    if catalog.get("private_reference_transform") != {
        ...
        "all_140_private_reference_hashes_unique": True,
        "all_140_public_control_hashes_unique": True,
        "purpose": (
        ...
        raise RuntimeError("r5 private reference-transform contract drift")

    expected_families = {
        "artifact-fine-grain": ("artifact", [-1, 1], 12, 24),
        "artifact-speck": ("artifact", [-1, 1], 12, 24),
        "artifact-microblob": ("artifact", [-1, 1], 12, 24),
        "artifact-short-dash": ("artifact", [-1, 1], 12, 24),
        "artifact-parallel-bundle": ("artifact", [-1, 1], 12, 24),
    ...
    if not isinstance(families, list) or len(families) != len(expected_families):
        raise RuntimeError("r5 control family catalog drift")
    ...
    if len(family_ids) != len(families) or set(family_ids) != set(expected_families):
        raise RuntimeError("r5 control family id drift")
    for family in families:
        ...
        require_exact_keys(family, expected_keys, f"r5 family {family_id}")
        if (
    ...
    if (
        ...
        raise RuntimeError("r5 metric-window contract drift")
    selection = value.get("threshold_selection", {})
    if (
        ...
        raise RuntimeError("r5 endpoint private-role eligibility drift")
    public_policy = value.get("public_identity_policy", {})
    if (
        set(public_policy.get("forbidden_fields", []))
        != FORBIDDEN_PUBLIC_IDENTITY_FIELDS
    ):
        raise RuntimeError("r5 public identity/payload leak allowlist drift")
    for field in (
        "blindness_scope",
        "review_surface",
        ...
        if not isinstance(public_policy.get(field), str) or not public_policy[field]:
            raise RuntimeError(f"r5 public identity policy missing {field}")
    if (
        value["contact_sheets"].get("all_140_source_panels_unique_in_every_view")
        is not True
    ):
        raise RuntimeError("r5 public panel uniqueness contract drift")


def load_spec() -> dict[str, Any]:
    payload = SPEC_PATH.read_bytes()
    if sha256_bytes(payload) != SPEC_SHA256:
        raise RuntimeError("r5 preregistered spec SHA drift")
    value = json.loads(payload.decode("utf-8"))
    if (
        value.get("schema_version") != "microtexture-v2-r5-preregistered-spec/2"
        or not all(
        ...
        raise RuntimeError("r5 preregistration authority flag/schema drift")
    validate_preregistered_spec(value)
    return value


def blind_key() -> bytes:
    value = os.environ.get("MICROTEXTURE_V2_R5_BLIND_KEY")
    if value is None:
        raise RuntimeError("MICROTEXTURE_V2_R5_BLIND_KEY is required")
    if re.fullmatch(r"(?:[0-9a-f]{64}|[0-9A-F]{64})", value) is None:
        raise RuntimeError(
            "MICROTEXTURE_V2_R5_BLIND_KEY must be exactly 64 all-lowercase "
...
def _git(code_root: Path, *arguments: str) -> bytes:
    ...
    environment.pop("MICROTEXTURE_V2_R5_BLIND_KEY", None)
...
def artifact_root(repository: Path) -> Path:
    raw = os.environ.get("MICROTEXTURE_V2_R5_ARTIFACT_ROOT")
    if raw is None:
        raise RuntimeError("MICROTEXTURE_V2_R5_ARTIFACT_ROOT is required")
    required_relative = "tmp/map-production/microtexture-v2-r5-artifacts"
...
def validate_stage_completion_structure(value: Any, context: str) -> None:
    ...
    if (
        value["artifact"] != "microtexture-v2-r5-stage-completion"
        or value["schema_version"] != "microtexture-v2-r5-stage-completion/2"
...
def write_stage_completion_exclusive(
    *,
    ...
    completion = {
        "artifact": "microtexture-v2-r5-stage-completion",
        "schema_version": "microtexture-v2-r5-stage-completion/2",
        "stage": stage,
...
def validate_implementation_bindings() -> dict[str, Any]:
    ...
    if (
        value["artifact"] != "microtexture-v2-r5-implementation-bindings"
        or value["schema_version"] != "microtexture-v2-r5-implementation-bindings/2"
        or value["authority"] is not True
        or value["spec_sha256"] != SPEC_SHA256
    ):
        raise RuntimeError("r5 implementation binding header drift")
    expected = set(load_spec()["authority_files"]) - {"implementation-bindings.json"}
    if set(value["files"]) != expected:
        raise RuntimeError("r5 implementation binding file set drift")
    for relative, expected_sha in value["files"].items():
        ...
        if (
            ...
            raise RuntimeError(f"r5 implementation SHA drift: {relative}")
    return value
...
def assert_head_unchanged(captured_head: str) -> None:
    if _git(CODE_ROOT, "rev-parse", "HEAD").decode().strip() != captured_head:
        raise RuntimeError("Git HEAD changed during r5 operation")
...
def assert_git_ancestor(ancestor: str, descendant: str) -> None:
    try:
        ...
        raise RuntimeError(
            "formal calibration HEAD is not an ancestor of the current r5 HEAD"
        ) from error
...
def verify_tracked_foundation_corpus_provenance(
    ...
    for foundation in corpus["foundations"]:
        ...
        if sha256_bytes(payload) != foundation["sha256"]:
            raise RuntimeError(f"r5 foundation {foundation['id']} tracked SHA drift")
    ...
    for path_field, hash_field in (
        ...
        if sha256_bytes(payload) != corpus[hash_field]:
            raise RuntimeError(f"r5 foundation {path_field} tracked SHA drift")
    for index, review in enumerate(corpus["independent_vision_reviews"]):
        ...
        if sha256_bytes(payload) != review["sha256"]:
            raise RuntimeError(
                f"r5 foundation independent review {index} tracked SHA drift"
            )
    return payloads
...
def validate_vision_labels_payload(
    ...
    if (
        value["artifact"] != "microtexture-v2-r5-root-vision-labels"
        or value["schema_version"] != "microtexture-v2-r5-root-vision-labels/2"
...
def load_control_manifest(
    ...
    *,
    ...
    if (
        manifest["artifact"] != "microtexture-v2-r5-control-manifest"
        or manifest["schema_version"] != "microtexture-v2-r5-control-manifest/3"
...
def evaluate_endpoints_from_measurements(
    ...
    for endpoint in spec["threshold_selection"]["endpoint_definitions"]:
        codes = [
            code
            for code in endpoint_population_codes(endpoint["population"], labels)
            if code in clusters
        ]
...
def select_hard_threshold_from_measurements(
    ...
    for threshold in threshold_candidates(
        [float(measured[code]["metrics"][metric]) for code in sorted(clusters)]
    ):
...
def _calibration_candidate_thresholds(
    ...
    return threshold_candidates(
        [
            ...
            if item["anonymous_code"] in eligible
        ]
    )
...
def validate_calibration_report_nested(report: Any, spec: dict[str, Any]) -> None:
    ...
    if (
        report["artifact"] != "microtexture-v2-r5-calibration-report"
        or report["schema_version"] != "microtexture-v2-r5-calibration-report/2"
...
def validate_holdout_report_nested(
    ...
    if (
        report["artifact"] != "microtexture-v2-r5-holdout-report"
        or report["schema_version"] != "microtexture-v2-r5-holdout-report/2"
...
def validate_locked_clean_reference_report_nested(
    ...
    if (
        report["artifact"] != "microtexture-v2-r5-locked-clean-reference-report"
        or report["schema_version"]
        != "microtexture-v2-r5-locked-clean-reference-report/2"
        or report["spec_sha256"] != SPEC_SHA256
    ):
        raise RuntimeError(f"{context} identity/schema drift")
    _validate_normal_report_flags(report, context)
...
def load_calibration_report(
    ...
    if report["passed"]:
        ...
        if (
            frozen["artifact"] != "microtexture-v2-r5-thresholds-frozen"
            or frozen["schema_version"] != "microtexture-v2-r5-thresholds/2"
...
def load_locked_clean_reference_report(
    ...
    if (
        locked["artifact"] != "microtexture-v2-r5-locked-clean-reference-report"
...
def load_threshold_authority_receipt(
    ...
    if (
        receipt["artifact"] != "microtexture-v2-r5-threshold-authority"
