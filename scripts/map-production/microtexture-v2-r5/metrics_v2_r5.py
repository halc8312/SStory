"""Deterministic bounded r5 residual-window metrics.
...
def _bounded_ratio(value: float, reference: float) -> float:
    """Monotone saturation with ``reference`` mapping to exactly 0.5."""
...
def _score_references(definition: dict[str, Any]) -> dict[str, float]:
    ...
    if not isinstance(source, dict) or set(source) != REFERENCE_KEYS:
        raise ValueError(
            "score_reference_constants must contain the exact r5 reference keys"
        )
    return {
...
def recompute_branch_scores(
    ...
    if missing:
        raise ValueError(f"missing raw r5 metrics: {missing}")
    ...
    for key in RAW_INTEGER_FIELDS:
        ...
        if int(value) < 0:
            raise ValueError(f"raw_metrics.{key} must be a non-negative integer")
        raw[key] = float(value)
    if raw["parallel_pair_ratio"] > 1.0:
        raise ValueError("raw_metrics.parallel_pair_ratio must be within 0..1")

    grain_rms = _bounded_ratio(raw["grain_rms_l"], references["grain_rms_l"])
    grain_dense = min(
        _bounded_ratio(
    ...
    grain_coherent = min(
        _bounded_ratio(
            raw["grain_coherence_2_to_13"],
            references["grain_coherence_2_to_13"],
        ),
        grain_rms,
    )
    ...
    spot_score = max(
        _bounded_ratio(raw["tiny_mass_l"], references["tiny_mass_l"]),
        _bounded_ratio(
    ...
    finite_line_score = max(
        _bounded_ratio(raw["finite_line_peak_l"], references["finite_line_peak_l"]),
        _bounded_ratio(
    ...
    parallel_bundle_score = min(
        _bounded_ratio(raw["parallel_pair_peak_l"], references["parallel_pair_peak_l"]),
        _bounded_ratio(
    ...
    if any(
        ...
        raise RuntimeError("non-finite or unbounded r5 branch score")
    return result
...
def _line_kernel(
    ...
    if not core_count or not flank_count:
        raise RuntimeError("empty r5 finite-line kernel support")
    ...
    if abs(float(kernel.sum(dtype=np.float64))) > 1e-6:
        raise RuntimeError("r5 finite-line kernel lost zero-sum contract")
...
def _line_bank(
    ...
    if angles != list(range(0, 180, 15)):
        raise ValueError("r5 finite-line angles must be exactly 0..165 by 15 degrees")
    if lengths != [5.0, 9.0, 15.0, 23.0]:
        raise ValueError("r5 finite-line lengths must be exactly 5, 9, 15, 23 px")
...
def _parallel_core_kernel(
    ...
    core_count = int(np.count_nonzero(core))
    if not core_count:
        raise RuntimeError("empty r5 parallel core kernel support")
    kernel = np.zeros(along.shape, dtype=np.float32)
    kernel[core] = np.float32(1.0 / core_count)
    kernel.setflags(write=False)
    return kernel
...
def _parallel_metrics(
    ...
    for angle_degrees, length, response in bank:
        ...
        if pair_count < minimum_pair_count:
            ...
            pair_count = 0
        score = min(
            _bounded_ratio(pair_peak, references["parallel_pair_peak_l"]),
            _bounded_ratio(pair_count, references["parallel_matched_pair_count"]),
        )
...
def measure(
    ...
    if (
        ...
        raise ValueError(
            f"r5 metric input must be exact HxW {expected_shape}; "
    ...
    if not np.all(np.isfinite(control_float)) or not np.all(
        ...
        raise ValueError("r5 metric input must be finite")
    delta = control_float - reference_float
    if not np.all(np.isfinite(delta)):
        raise ValueError("r5 metric delta must be finite")
    if not np.any(delta):
    ...
    if set(result) != METRIC_FIELDS:
        raise RuntimeError("r5 metric field-set drift")
    if any(
        isinstance(value, float) and not math.isfinite(value)
        for value in result.values()
    ):
        raise RuntimeError("non-finite r5 metric")
    return result
