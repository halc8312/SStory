"""r4 blinded calibration, locked-clean validation, and one-shot holdout."""
...
def _load_manifest(split: str, state: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    for index, record in enumerate(manifest["records"]):
        ...
        exact_artifact_path_without_links(
            root,
            root / expected_control_path,
            expected_control_path,
            must_exist=True,
        )
        exact_artifact_path_without_links(
            root,
            root / expected_reference_path,
...
def _exact_metric_window(values: np.ndarray, spec: dict[str, Any]) -> np.ndarray:
    ...
    if values.shape != expected_canvas:
        raise RuntimeError(
            f"r4 control canvas drift: {values.shape} != {expected_canvas}"
        )
    x, y, width, height = [
    ...
    if crop.shape != expected:
        raise RuntimeError("r4 exact metric-window crop drift")
    return crop
...
def _measure_records(
    ...
    for record in manifest["records"]:
        ...
        control = _exact_metric_window(
            _decode_image_once(
                exact_artifact_path_without_links(
                    root,
                    root / record["control_png"],
                    record["control_png"],
                    must_exist=True,
                ),
                record["control_sha256"],
        ...
        reference = _exact_metric_window(
            _decode_image_once(
                exact_artifact_path_without_links(
                    root,
                    root / record["reference_png"],
                    record["reference_png"],
                    must_exist=True,
                ),
                record["reference_sha256"],
                "L",
...
def _diagnostic_flags(measured: dict[str, dict[str, Any]]) -> dict[str, list[str]]:
    levels = load_spec()["metric_definition"][
    ...
    for code, record in measured.items():
        ...
        if float(metrics["sparse_blob_score"]) > float(levels["sparse_blob_score"]):
            names.append("sparse-blob-diagnostic")
        line = float(metrics["finite_line_score"]) > float(levels["finite_line_score"])
        if line:
            names.append("finite-line-diagnostic")
        if line and float(metrics["parallel_pair_ratio"]) > float(
            levels["parallel_pair_ratio"]
        ):
            names.append("parallel-pair-diagnostic")
        flags[code] = names
    return flags
...
def _write_one_shot_failure_report(
    *,
    ...
    report = {
        "artifact": "microtexture-v2-r4-one-shot-failure-report",
        "schema_version": "microtexture-v2-r4-one-shot-failure-report/2",
        "stage": stage,
...
def _record_post_marker_failure(
    *,
    ...
    try:
        ...
        try:
            error.add_note(
                "r4 failure-report persistence also failed; the exclusive marker "
...
def calibrate(labels_path: Path) -> dict[str, Any]:
    ...
    marker_sha = write_json_exclusive(
        root,
    ...
    try:
        ...
        if passed and hard_threshold is not None:
            frozen = {
                "artifact": "microtexture-v2-r4-thresholds-frozen",
                "schema_version": "microtexture-v2-r4-thresholds/2",
                "authority": True,
                "spec_sha256": SPEC_SHA256,
                "blind_key_commitment": state["blind_key_commitment"],
    ...
    if not passed or hard_threshold is None:
        raise RuntimeError(f"calibration failed and r4 is closed: {status}")
    return {
        "passed": True,
        "blind_key_commitment": state["blind_key_commitment"],
...
def validate_locked_clean_reference() -> dict[str, Any]:
    ...
    marker_sha = write_json_exclusive(
        root,
        root / "markers/locked-clean-reference-validation-started.json",
        marker,
    )
    ...
    try:
        ...
        raise
    if not accepted:
        raise RuntimeError("locked-clean-reference failed and r4 is closed")
    return {"passed": True}
...
def holdout(labels_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise
    if not passed:
        raise RuntimeError("holdout failed and r4 is closed")
    return {"passed": True}
