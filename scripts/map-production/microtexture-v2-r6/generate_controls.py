"""Generate operationally blinded r6 calibration or authorized holdout sheets."""
...
def _validate_private_controls(controls: list[Any]) -> None:
    if len(controls) != 220:
        raise RuntimeError("r6 generator requires exactly 220 private controls")
    roles = Counter(control.private_role for control in controls)
    if roles != Counter({"artifact": 200, "protocol-zero": 16, "duplicate-audit": 4}):
        raise RuntimeError("r6 generator private-role contract drift")
    if {control.foundation_id for control in controls} - {"v15", "v16", "v17"}:
        raise RuntimeError("r6 generator rejected foundation assignment")
    groups = Counter(
    ...
    if groups != Counter({"clean": 2, "artifact": 2}):
        raise RuntimeError("r6 generator duplicate-audit contract drift")
    if any(
        control.duplicate_audit_group is not None
        for control in controls
        if control.private_role != "duplicate-audit"
    ):
        raise RuntimeError("r6 generator duplicate group leaked across roles")
