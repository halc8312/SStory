...
fresh `dev-r11` を次の一回限りdevelopment probeとして事前登録します。exact rootは
...
schedule revisionは`dev-r11-grain-coherence-support-schedule-v1`です。規範JSON、bindings、code、tests、runnerを
別commitへfreezeし、push後にUbuntu/Windows CIを両方通すまでgenerateしてはいけません。formal keyの
非永続化契約は変更しません。
...
継承・公開しません。closed dev-r8/dev-r9/dev-r10 keyは各Git-ignored private rootにだけ保持して再利用せず、
planned dev-r11 keyは別のfresh Git-ignored rootとtracked custodian runnerだけが扱います。marker 前の
...
変更しないcoherence metric support `2..13`のguard-bandedな内側`3..12`へ入ります。ただしdev-r10はgeneration中断で
Vision/analysisへ到達しておらず、このpartial corpusからscheduleを評価・調整しません。dev-r11はこのmorphology、
speck coverage、metric、score reference、threshold rule、tier数`5/4/7/4`、全endpoint count/rate、他4 familyを
一切変更せず、fresh identityだけで全20 conditionsと各split 220 recordsを一体生成・blind reviewし、全population
gateをall-or-nothingで適用します。
...
closed `dev-r8` / `dev-r9` とplanned `dev-r11` の一回限りのanalysisには、formal specのendpoint最低populationを
一切変更せず、さらに厳しいdevelopment-only safety floorを上乗せします。dev-r10はanalysis前に閉鎖したため、この
gateの結果を持ちません。label bytesを両splitともsealし、private identityを
...
references、pixels、identities、opaque codes、labels、measurementsはdev-r9、dev-r10、dev-r11、formalへ再利用しません。

dev-r9はこのgateを両splitで通過しましたが、calibrationでendpoint-admissible thresholdが存在せず、測定後に
閉鎖しました。dev-r10はgeneration中断のためgateへ到達していません。planned dev-r11でもpopulationが一つでも
...
明示的なnon-formal development keysでfreezeします。closed `dev-r6`～`dev-r10` corporaは正式判断に使いません。
dev-r10はgeneration中断でVision/analysis evidenceを持たず、partial outputをschedule選択へ利用しません。planned
`dev-r11` の`dev-r11-grain-coherence-support-schedule-v1`全scheduleをsubsettingなしで一回だけ確認します。
...
閉鎖しました。planned dev-r11はmetricを変更せず継承し、dev-r7/dev-r9のthreshold、diagnostic、measurement、
dev-r10のpartial generation outputをformalへ使用しません。
...
## Development closure and planned dev-r11 operator order
...
dev-r11はformal r6ではなく、formal前の新しい一回限りdevelopment probeとしてのみ計画されています。このREADMEの
記述だけでは実行権限になりません。`dev-r11-grain-coherence-support-schedule-v1`、新しいroot/key、全nonce/domain、
...
共存、summary / seal / completionの欠落、hard killを含む中断はdev-r11を消費・閉鎖します。その後に限り、
Root/独立Visionによる全37
...
選び、その値をholdoutへ変更せず適用します。一つでも失敗すればdev-r11を消費・閉鎖し、rerun、resume、relabel、
別key、subsetting、top-upをしません。成功時もdev-r11 thresholdはformal authorityではなく、success auditをGitへ
固定してdev-r11を閉じた後にのみ、formal r6 authorityを別commitで最終freezeできます。

## Formal operator order

dev-r11が上記の全population gate、metric、calibration、holdout endpointsを一回でpassし、hash-bound success auditを
