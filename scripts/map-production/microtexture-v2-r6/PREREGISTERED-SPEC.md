...
です。calibrationのclean / obvious-artifact duplicate groupsとholdoutのclean duplicate groupはpassしました。calibration obvious pairは両方`reject`、severity `3`、visible flagは`l`のみ、holdout obvious pairは両方`clean`、severity `0`、visible flagなしでした。population aggregation、numeric measurement、metric evaluation、threshold searchは未開始で、read-only sanitized postmortemは一度だけです。auditは
`world/map-production/qa/microtexture-v2-r6-dev-r19-development-failure.json`、raw SHA-256は
`96d93fe63be2ff6171ade926dbace188b6fd5eacf748a6f03a787781a5d248d0`です。`dev-r19`の全root / key / private material / outputは後続editionまたはformalへ再利用しません。
...
です。両private auditはpassしましたが、premeasurement population auditでcalibration / holdoutのtiny-speck population `0` / `1`がformal minimum `4`とdevelopment floor `6`をともにmissしました。他の全endpointは両minimumをpassしました。numeric metricとthreshold searchを開始せず閉鎖し、read-only sanitized postmortemは一度だけです。auditは`world/map-production/qa/microtexture-v2-r6-dev-r20-development-failure.json`、raw SHA-256は`e8689321135e8c5d3fb038fbaa7c3ccbe644999905f4a3d3834fa30969ff27c8`です。dev-r20のsuccessorだったdev-r21も閉鎖済みで、current successorはfresh development-only `dev-r22`です。

`dev-r20`はschedule / sentinel revisionsは`dev-r20-strong-finite-duplicate-short-line-sentinel-schedule-v1` /
`dev-r20-keyed-axial-short-line-duplicate-sentinel-v1`です。obvious-artifact duplicateだけを当該editionのfresh-keyed positive `+12.0 L`のaxial bar `12`本へ変更しました。各barはencoded `24×3 px`、各exact metric quadrantに`3`本、各quadrantにhorizontal / verticalの両方を含み、center間Chebyshev distanceは`>=32 px`、support guardは`>=2 px`、nonzero supportは`864 px`です。duplicate pairはrequested delta、decoded residual、metricがexactに等しく、reference、control、anonymous code、control IDは異なります。geometry contractと両private auditはpassしましたが、artifact population全体のVision truthは保証されず、tiny-speck population gateをfailしました。

clean duplicate、`dev-r19`のreject severity-band policy、全200 artifact morphologies、全design tier、metric、threshold、population、rate contractは不変です。full artifact morphology / sanitized r19 basis / sentinel manifest SHA-256は順に
...
`dev-r21`のstatusは`failed-and-closed-before-analysis`です。generation、両reviewerのblind `440 × 2` review、immutable initial snapshot / receipt、bilateral reconciliationは一度だけ完了しました。一回のofficial public preflightはmachine receiptのないoperator observationで、calibration Rootの`decisions-root.initial.dev.txt` 6行目にある非canonical `lp`（required `l,p`）をstrict parserが`decision DSL flag drift`として拒否した時点で停止しました。`lp`の完全scopeは両splitともRoot initial `33`、Independent initial `32`、vision / Root / Independent finalそれぞれ`32`です。parserの弱さではなく、receipt sealing前にparserを強制する専用writerがなかったことがroot causeです。

label seal、key read、private reveal、private / population audit、analysis、numeric measurement、threshold search、postmortemは未開始です。key bytesは読まず、logにもGitにも記録しません。closed root、sealed initial snapshots / receipts、reconciled finalsは変更せず保持し、13個のcalibration review-crop derivativeを含む全dev-r21 derivativeとroot / key / private / public materialを再利用しません。auditは`world/map-production/qa/microtexture-v2-r6-dev-r21-development-failure.json`、raw / canonical SHA-256は`89229d2b5f0b6c91c61cea6547127cc4441fd7cc6bd02d6cc17a3c7aec2d61bd` / `ffa5759cd56417deb1e6c53310e52f73ecdd6fe7929129939b464d0c049542cd`です。

fresh development-only `dev-r22`はdev-r21の全200 artifact morphology、duplicate、tier、minima、metric、threshold、population、rateをexactに保持し、morphology / numeric requirementを変更しません。schedule revisionは`dev-r22-r21-morphology-strict-preseal-decision-schedule-v1`です。fresh root / keyは`tmp/map-production/microtexture-v2-r6-dev-r22` / `tmp/map-production/microtexture-v2-r6-dev-r22/private/development-key.bin`、public noncesは`r6-calibration-v17` / `r6-holdout-v17`です。condition-cluster / render-seed / opaque-code / private-reference-transformはv17、public commitmentはv18、key commitment / foundation-offset / foundation-assignment / delta / private-control-idはv16です。protocol-zero / artifact basesは`1451000` / `1461000` / `1473000` / `1483000`、duplicate noncesは`1491000..1491002` / `1501000..1501002`です。

各initial snapshotはreconciliation前に専用`seal-initial-decisions --split <split> --reviewer <reviewer>`を通します。このwriterだけがreceiptをexclusive createし、同じstrict parser、canonical comma-separated `g,t,b,l,p`、220-record coverage、page/rowとanonymous-code bindingをpassしたexact bytesだけをsealします。不正な`lp`、順序、重複、空token、coverage/code driftではreceiptを作りません。四つのreceiptが揃ってからreconciliation / official preflightへ進みます。

dev-r22 generation、review、preseal、reconciliation、preflight、label seal、private reveal、measurement、threshold、auditは未開始です。同一authority commitのpushとUbuntu / Windows CI両方のgreenがgeneration開始条件です。sanitized r21 basis / preserved morphology / schedule keyset / changed-values / catalog / probe SHA-256は`62dc7c4e5f31211cd5ea2fa7cbb43ebc2944e7e88bf1db79e6c1b4b738843e86` / `99aa3643bdddc0cd1257cbda0b5784cf08f90b80f947ef34ca90c71093046595` / `428d7dcbf3aa796bfa419cd1f0e852f0207a52a6971ccb329c82a075abbeda07` / `ee63cf45a728caa0c1635ddf41b89ee60d864ea5f3787890a036c877a96bfc60` / `4ba48c2cfbea7299d3ae596675fe5a97f92fc4520f5fb68181d35e8d418419af` / `d579abd7b2ed596f699c2bd5ce2e30db43d143f29a1b7315053184e632fe0e9b`です。formal stageはblockedです。
...
RootとIndependentは各splitのofficial initial snapshot
`decisions-root.initial.dev.txt` / `decisions-independent.initial.dev.txt`をauthorしますが、`.sha256` receiptを手作業で作ってはいけません。reconciliation前に専用`seal-initial-decisions --split <split> --reviewer <reviewer>`を実行し、同じofficial parser、canonical comma-separated flag order、220-record coverage、page/rowとanonymous-code bindingをpassした場合だけ、writerが`lowercase-sha256 two-spaces snapshot-basename newline` receiptをexclusive createします。失敗時はreceiptを作らず、snapshot修正後に未sealのまま再検証します。reconciliation後の`vision-decisions.dev.txt` / `decisions-root.dev.txt` /
...
dev-r19はgeneration、`440 × 2` blind review、bilateral reconciliation、official preflight、label sealing、private reveal、regeneration、protocol-zero auditを各一度だけ完了しました。calibrationのclean / obvious-artifact duplicate groupsとholdoutのclean duplicate groupはpassしましたが、holdout obvious-artifact pairは両方`clean`、severity `0`、visible flagなしで、必須の`reject`、severity `2` / `3`、`short_line_visible=true`契約をfailしました。population auditとnumeric metricを開始せず`failed-and-closed-before-population-audit`で閉鎖し、read-only postmortemは一度だけです。auditは`world/map-production/qa/microtexture-v2-r6-dev-r19-development-failure.json`、raw SHA-256は`96d93fe63be2ff6171ade926dbace188b6fd5eacf748a6f03a787781a5d248d0`です。dev-r19の全素材・outputは再利用できません。

dev-r20はgeneration、`440 × 2` blind review、bilateral reconciliation、official preflight、label sealing、private reveal、regeneration、両private audit、premeasurement population auditを各一度だけ実行しました。両private auditはpassしましたが、calibration / holdout tiny-speck population `0` / `1`がformal minimum `4`とdevelopment floor `6`をともにmissし、他の全endpointは両minimumをpassしました。numeric metricとthreshold searchを開始せず`failed-and-closed-before-measurement`で閉鎖し、read-only postmortemは一度だけです。auditは`world/map-production/qa/microtexture-v2-r6-dev-r20-development-failure.json`、raw SHA-256は`e8689321135e8c5d3fb038fbaa7c3ccbe644999905f4a3d3834fa30969ff27c8`です。dev-r20の全素材・outputは再利用できません。dev-r21もpublic preflight failureでanalysis前に閉鎖し、fresh development-only dev-r22をpreregisterしました。dev-r22 generation、Vision、measurement、auditは未開始です。

## Formal stage order

dev-r20は両private auditをpassしましたが両splitのtiny-speck population gateをfailし、numeric measurement前に閉鎖しました。dev-r21もstrict public decision parserが`lp`を拒否した時点でanalysis前に閉鎖しました。
fresh development-only dev-r22は登録済みですがgeneration前であり、formal authorityではありません。以下のformal stageはすべてblockedです。dev-r21以前またはdev-r22のroot、key、controls、labels、decisions、measurements、threshold、public surfaces、postmortem outputをformal authorityへ流用できません。
