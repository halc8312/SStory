...
です。schedule / overlay revisionsは`dev-r21-symmetric-hard-point-speck-population-schedule-v1` / `dev-r21-reject-speck-encoded-hard-plus-v1`です。各splitのreject-tier artifact-speck 10条件だけを、diameter `1`、center amplitude `12.0 L`、shoulder fraction `0.92`（encoded `12 L` center + encoded `11 L` axial shoulders 4点）、count `12..21`、split/conditionごとに固定したminimum separation `30..37`へ置換します。calibrationの`index:count/separation`は`3:12/30, 5:13/32, 7:14/34, 8:15/31, 15:16/33, 19:17/35, 6:18/30, 12:19/32, 16:20/34, 17:21/36`、holdoutは`3:12/31, 4:13/33, 6:14/35, 10:15/32, 11:16/34, 18:17/36, 1:18/31, 9:19/33, 13:20/35, 14:21/37`です。他180 artifact morphologies、全tier cardinality、dev-r19 reject-band duplicate policy、r20のclean / obvious-artifact duplicate constructions、formal/development population minima、metric、threshold、rateを不変にします。Vision truthまたはgate通過を保証しません。

fresh root / keyは`tmp/map-production/microtexture-v2-r6-dev-r21` / `tmp/map-production/microtexture-v2-r6-dev-r21/private/development-key.bin`です。public noncesは`r6-calibration-v16` / `r6-holdout-v16`、condition-cluster / render-seed / opaque-code / private-reference-transformはv16、public payload commitmentはv17、key commitment / foundation-offset / foundation-assignment / delta / private-control-idはv15です。protocol-zero nonce basesは`1351000` / `1361000`、artifact basesは`1373000` / `1383000`、duplicate-audit noncesは`1391000..1391002` / `1401000..1401002`です。keyは`secrets.token_bytes(32)`で一度だけ新規生成し、全root、key、domains、nonces、controls、references、identities、codes、commitments、labels、decisions、measurements、outputsをr20以前から再利用しません。

この変更はauthority登録だけです。dev-r21 generation、Root/Independent Vision、reconciliation、official preflight、label seal、private reveal、measurement、threshold search、success/failure auditは未開始で、生成物・decision・audit outputは存在しません。exact authority commitのpushと同一commitのUbuntu / Windows CI両方のgreenがgeneration開始条件です。dev-r21はformal authorityではなく、formal stageはblockedのままです。
