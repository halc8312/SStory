"""r6 blinded calibration, locked-clean validation, and one-shot holdout."""
...
from metrics_v2_r6 import measure
...
def _eligible_condition_clusters(
    identities: dict[tuple[str, str, str, str], ExpectedControl],
    spec: dict[str, Any],
) -> dict[str, str]:
    ...
    if not clusters:
        raise RuntimeError("eligible artifact condition clusters are empty")
    return clusters
...
def _exact_metric_window(values: np.ndarray, spec: dict[str, Any]) -> np.ndarray:
    ...
    if values.shape != expected_canvas:
        raise RuntimeError(
            f"r6 control canvas drift: {values.shape} != {expected_canvas}"
        )
    x, y, width, height = [
    ...
    if crop.shape != expected:
        raise RuntimeError("r6 exact metric-window crop drift")
    return crop
...
def _diagnostic_flags(measured: dict[str, dict[str, Any]]) -> dict[str, list[str]]:
    cutoff = float(load_spec()["metric_definition"]["diagnostic_branch_flag_cutoff"])
    ...
    for code, record in measured.items():
        ...
        for metric, name in (
            ...
            if float(metrics[metric]) > cutoff:
                names.append(name)
        flags[code] = names
    return flags
...
def _write_one_shot_failure_report(
    *,
    ...
    report = {
        "artifact": "microtexture-v2-r6-one-shot-failure-report",
        "schema_version": "microtexture-v2-r6-one-shot-failure-report/2",
        "stage": stage,
...
def _record_post_marker_failure(
    *,
    ...
    try:
        ...
        try:
            error.add_note(
                "r6 failure-report persistence also failed; the exclusive marker "
...
def calibrate(labels_path: Path) -> dict[str, Any]:
    ...
    with _one_shot_stage_guard(
        ...
        phase = "validate-endpoint-population-feasibility"
        prerevealed_clusters = _eligible_condition_clusters(identities, load_spec())
        validate_endpoint_population_counts(labels, prerevealed_clusters, load_spec())
        phase = "measure-calibration-records"
        measurements, reveal, clusters = _measure_records(manifest, identities, state)
        if clusters != prerevealed_clusters:
            raise RuntimeError("calibration eligible condition-cluster drift")
        ...
        report = {
            "artifact": "microtexture-v2-r6-calibration-report",
            "schema_version": "microtexture-v2-r6-calibration-report/2",
            "spec_sha256": SPEC_SHA256,
            "blind_key_commitment": state["blind_key_commitment"],
            "manifest_sha256": manifest_sha,
            "labels_sha256": labels_sha,
        ...
        if passed and hard_threshold is not None:
            frozen = {
                "artifact": "microtexture-v2-r6-thresholds-frozen",
                "schema_version": "microtexture-v2-r6-thresholds/2",
                "authority": True,
                "spec_sha256": SPEC_SHA256,
                "blind_key_commitment": state["blind_key_commitment"],
    ...
    if not passed or hard_threshold is None:
        raise RuntimeError(f"calibration failed and r6 is closed: {status}")
    return {
        "passed": True,
        "blind_key_commitment": state["blind_key_commitment"],
...
def validate_locked_clean_reference() -> dict[str, Any]:
    ...
    if not accepted:
        raise RuntimeError("locked-clean-reference failed and r6 is closed")
    return {"passed": True}
...
def holdout(labels_path: Path) -> dict[str, Any]:
    ...
    with _one_shot_stage_guard(
        ...
        measurements, reveal, clusters = _measure_records(manifest, identities, state)
        if clusters != prerevealed_clusters:
            raise RuntimeError("holdout eligible condition-cluster drift")
        measured = {record["anonymous_code"]: record for record in measurements}
        threshold = frozen["hard_threshold"]
        ...
        report = {
            "artifact": "microtexture-v2-r6-holdout-report",
            "schema_version": "microtexture-v2-r6-holdout-report/2",
            "authority": True,
            "spec_sha256": SPEC_SHA256,
            "blind_key_commitment": state["blind_key_commitment"],
            "manifest_sha256": manifest_sha,
    ...
    if not passed:
        raise RuntimeError("holdout failed and r6 is closed")
    return {"passed": True}
