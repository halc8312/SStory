...
DEV_ROOT = REPO_ROOT / "tmp" / "map-production" / "microtexture-v2-r6-dev-r21"
...
_R21_MATERIALIZED_SPEC_CHANGED_PATHS = {
    "history.dev_r21_status": common.DEV_R21_STATUS,
    "history.dev_r21_role": common.DEV_R21_ROLE,
    "metric_definition.development_basis": common.R21_DEVELOPMENT_BASIS,
    "development_probe_secret_handling.scope": common.DEV_R21_SECRET_SCOPE,
    ...
    "public_identity_policy.reviewer_access_contract": (
        common.DEV_R21_REVIEWER_ACCESS_CONTRACT
    ),
}
...
def _development_blind_commitment(
    key: bytes, message: str = _R21_KEY_COMMITMENT_MESSAGE
) -> str:
    if len(key) != 32:
...
def _load_spec() -> tuple[dict[str, Any], str]:
    ...
    _validate_dev_r21_spec_authority(value)
    return value, digest
...
def _require_same_head_dual_ci_green(captured_head: str) -> None:
    ...
    for run in check_runs:
        if not isinstance(run, dict) or run.get("name") not in _R21_REQUIRED_CI_CHECKS:
            continue
        name = run["name"]
    ...
    if set(latest_by_name) != set(_R21_REQUIRED_CI_CHECKS) or any(
...
def generate() -> None:
    ...
    state = {
        "development_edition": DEVELOPMENT_EDITION,
        "development_authority_sha256": _R21_PROBE_AUTHORITY_MANIFEST_SHA256,
