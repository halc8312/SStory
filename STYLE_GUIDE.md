...
- 千単位: `1,000` (カンマ区切り)
- 小数点: `.` (例: `3.14`)
- 範囲: `～` (全角波線) または `-` (半角ハイフン) どちらでも可、統一推奨 (`-` を推奨)
...
- マイナス: `-` (半角)
- ダッシュ: `—` (em-dash) または `-` ( hyphen) 可、統一
- 引用: `"` (半角) または `「」` (日本語 citation)
...
| 国家 | 統一表記 | 単位 |
|------|----------|------|
| ゼフィア連合 | ゼフィア金貨 | 1金貨 = 100銀貨 = 10,000銅貨 |
...
- **辞書に登録された語**: `terminology/dictionary.yaml` に記載されたすべての語は、本文中で使用してはならない。該当する英語・中国語表記は指定の日本語表記に統一する。
- **許容される略称**: 以下の TRPG 標準略称は、辞書に登録がなく、日本語への変換を必須としない。英語略称のまま使用できる。
  - 能力値: `STR`, `DEX`, `CON`, `INT`, `WIS`, `CHA`
  - 防御値: `AC` (Armor Class)
  - ヒットポイント: `HP` (Hit Points)
  - 難易度: `DC` (Difficulty Class)
  - 経験値: `XP` (Experience Points)
  - ダイス: `d20`, `d6` など（数字と組み合わせの形）
- **単位**: フィート (`ft`) と通貨 (`GP`) は辞書に登録されており、それぞれ `フィート`、`金貨` に統一必須。
- **アイテム名・能力名**: `artifact` / `artefact`（魔導器）など、TRPG 固有の英語名は辞書に従い日本語に統一する。
...
- **emoji**: 使用可、控えめに (✅ ❌ ⚠️ 程度)
- **コメント**: Markdown内にコメント書かない (HTMLコメント可、削除対象)
