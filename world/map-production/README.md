...
dev-r18では各splitの既存reject-tier speck 10条件だけを対称に置換し、内訳はclear-reject 6条件 + dominant-reject 4条件でした。全targetは
...
dev-r18 rootは`tmp/map-production/microtexture-v2-r6-dev-r18`です。public noncesは`r6-calibration-v13` /
...
です。両private auditはpassしましたが、calibration / holdoutのtiny-speck population `0` / `1`がformal minimum `4`とdevelopment floor `6`をともにmissし、他の全endpointは両minimumをpassしました。numeric metricとthreshold searchを開始せず閉鎖し、read-only sanitized postmortemは一度だけです。auditは`world/map-production/qa/microtexture-v2-r6-dev-r20-development-failure.json`、raw SHA-256は`e8689321135e8c5d3fb038fbaa7c3ccbe644999905f4a3d3834fa30969ff27c8`です。dev-r20のpreregistered successorだったdev-r21も現在は閉鎖済みで、current successorはfresh development-only `dev-r22`です。

`dev-r20`はschedule / sentinel revisionsは`dev-r20-strong-finite-duplicate-short-line-sentinel-schedule-v1` /
`dev-r20-keyed-axial-short-line-duplicate-sentinel-v1`です。`obvious-artifact` duplicateのみを当該editionのfresh-keyed positive `+12.0 L`のaxial bar `12`本へ変更しました。各barはencoded `24×3 px`、各exact metric quadrantに`3`本、各quadrantにhorizontal / verticalの両方を含み、center間Chebyshev distanceは`>=32 px`、support guardは`>=2 px`、nonzero supportは`864 px`です。pairはrequested delta、decoded residual、metricがexactに等しく、reference、control、anonymous code、control IDは異なります。このgeometry contractと両private auditはpassしましたが、アーティファクトpopulation全体のVision truthは保証されず、tiny-speck population gateをfailしました。
...
`dev-r21`のstatusは`failed-and-closed-before-analysis`です。generation、両reviewerのblind `440 × 2` review、immutable initial snapshots / receipts、bilateral reconciliationを一度だけ完了しました。一回のofficial public preflightはmachine receiptのないoperator observationで、calibration Root initial line 6の非canonical `lp`（required `l,p`）をstrict parserが`decision DSL flag drift`として拒否した時点で停止しました。両splitの完全scopeはRoot initial `33`、Independent initial `32`、三つのfinal fileそれぞれ`32`です。label seal、key read / log / tracking、private reveal / audit、population audit、analysis、measurement、threshold search、postmortemは未開始です。closed rootを変更せず保持し、13個のcalibration review-cropを含む全derivativeを再利用しません。auditのraw / canonical SHA-256は`89229d2b5f0b6c91c61cea6547127cc4441fd7cc6bd02d6cc17a3c7aec2d61bd` / `ffa5759cd56417deb1e6c53310e52f73ecdd6fe7929129939b464d0c049542cd`です。

fresh development-only `dev-r22`はdev-r21の全200 artifact morphology、両duplicate、tier、minima、metric、threshold、population、rateを変更しません。scheduleは`dev-r22-r21-morphology-strict-preseal-decision-schedule-v1`です。root / keyは`tmp/map-production/microtexture-v2-r6-dev-r22` / `tmp/map-production/microtexture-v2-r6-dev-r22/private/development-key.bin`、public noncesは`r6-calibration-v17` / `r6-holdout-v17`です。condition-cluster / render-seed / opaque-code / private-reference-transformはv17、public commitmentはv18、key commitment / foundation lanes / delta / private-control-idはv16です。protocol-zero / artifact basesは`1451000` / `1461000` / `1473000` / `1483000`、duplicate noncesは`1491000..1491002` / `1501000..1501002`です。

各initial snapshotはreconciliation前に専用`seal-initial-decisions --split <split> --reviewer <reviewer>`を通し、同じstrict parser、canonical comma-separated flags、220-record coverage、page/row / anonymous-code bindingをpassした場合だけreceiptをexclusive createします。`lp`等のinvalid DSLではreceiptを作りません。四つのreceipt後にだけreconciliation / official preflightへ進みます。

dev-r22はgeneration前で、review、preseal、reconciliation、preflight、label、private、measurement、threshold、auditも未開始です。同一authority commitのpushとUbuntu / Windows CI両方のgreenがgeneration開始条件です。sanitized r21 basis / preserved morphology / catalog / probe SHA-256は`62dc7c4e5f31211cd5ea2fa7cbb43ebc2944e7e88bf1db79e6c1b4b738843e86` / `99aa3643bdddc0cd1257cbda0b5784cf08f90b80f947ef34ca90c71093046595` / `4ba48c2cfbea7299d3ae596675fe5a97f92fc4520f5fb68181d35e8d418419af` / `d579abd7b2ed596f699c2bd5ce2e30db43d143f29a1b7315053184e632fe0e9b`です。formal stageはblockedです。
