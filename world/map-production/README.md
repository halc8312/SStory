...
`dev-r21`のstatusは`fresh-development-only`、schedule / overlay revisionsは`dev-r21-symmetric-hard-point-speck-population-schedule-v1` / `dev-r21-reject-speck-encoded-hard-plus-v1`です。closed dev-r20のsanitized aggregateだけを根拠に、各splitのreject-tier アーティファクト-speck 10条件、計20条件だけをdiameter `1`、center `12.0 L`、shoulder fraction `0.92`（encoded center `12 L` / axial shoulders `11 L`）、count `12..21`、minimum separation `30..37`のhard-plusへ変更します。他180 morphology、両duplicate construction、全tier、formal/development minima、metric、threshold、rateは不変です。Vision truthまたはgate通過を保証しません。

fresh root / keyは`tmp/map-production/microtexture-v2-r6-dev-r21` / `tmp/map-production/microtexture-v2-r6-dev-r21/private/development-key.bin`です。public noncesは`r6-calibration-v16` / `r6-holdout-v16`、condition-cluster / render-seed / opaque-code / private-reference-transformはv16、public payload commitmentはv17、key commitment / foundation-offset / foundation-assignment / delta / private-control-idはv15です。protocol-zero nonce basesは`1351000` / `1361000`、アーティファクト basesは`1373000` / `1383000`、duplicate-audit noncesは`1391000..1391002` / `1401000..1401002`です。r20以前のroot / key / material / outputを一切再利用しません。

dev-r21はauthorityだけを登録したgeneration前の状態です。生成物、Vision decision、label、measurement、threshold、success/failure auditは存在しません。exact authority commitのpushと同一commitのUbuntu / Windows CI両方のgreen前にはgenerationを開始せず、formal stageもblockedのままです。
