---
type: "npc"
category: "npcs"
title: "冒険者ギルド 伝説の冒険者"
version: "1.0.0"
created: "2026-05-01"
last_updated: "2026-05-01"
author: "halc8312"
contributors: []
tags: ["npc", "adventurer", "hero", "legend"]
status: "stable"
npc_type: "adventurer"
race: "elf"
age: 52
alignment: "neutral-good"
class: "Ranger (Gloom Stalker) 10 / Rogue (Scout) 5"
spirit_contract:
  wind: 65
  earth: 30
  fire: 20
  water: 30
  moon: 40
---

# 冒険者ギルド 伝説の冒険者 (Legendary Adventurer)

**名前**: シルヴァン・グリーンウッド (Sylvan Greenwood)
**年齢**: 52歳 (エルフ)
**種族**: エルフ
**クラス**: Ranger (Gloom Stalker) 10 / Rogue ( Scout) 5
**alignment**: Neutral Good

**肩書き**: 冒険者ギルド最高ランク「SS」保持者、 Guild Master 候補
**所属**: 冒険者ギルド (全国支部)
**現状**: 現役引退間近だが、いまだ現役

---

## 外見

- **身長**: 172cm (エルフとして中肉)
- **髪**: 緑色短髪、戦闘時の脂ぎった髪
- **瞳**: 琥珀色、夜間は輝く
- **特徴**: 左目に古傷の傷跡、全身に無数の小さな傷、常に弓を背負う

---

## 性格

- **主要特性**: 沈着冷静、義理堅い、孤独好き、自然との調和
- **価値観**: 弱者保護、自然保護、ギルドの掟
- **目標**: 世界の脅威を排除、次世代の育成
- **弱点**: 集団行動が苦手、過去の失敗にこだわる

---

## 能力値

| 能力 | 値 | 修正 |
|------|----|------|
| **STR** | 14 | +2 |
| **DEX** | 20 | +5 |
| **CON** | 16 | +3 |
| **INT** | 14 | +2 |
| **WIS** | 16 | +3 |
| **CHA** | 12 | +1 |

**精霊契約**: 風 65% (狩猟の精霊と深い絆)

---

## 戦闘データ

**AC**: 18 ( leather armor + DEX + shield spell)
**HP**: 130 (Ranger d10×10 + Rogue d8×5 + CON×15)
**速度**: 35 フィート. (elf)

**攻撃**:
- **遠隔**: ロングボウ+2 (1d8+5 piercing, +2 enh, range 150/600)
- **近接**: 短剣 (1d4+2, 2 simultaneous)
- **特殊**: Dread Ambusher (追加攻撃 +1d8 damage on first turn)
- **隠密**: Stealth +9, 攻撃時にadvantage (from ambush)

**主な呪文/能力** (Ranger spells):
- Hunter's Mark (1d6 extra damage, concentration)
- Pass without Trace (+10 to Stealth)
- Conjure Animals (召喚狼)
- Cure Wounds (healing)

**特殊能力**:
- **Favored Enemy**: Monstrosity +2d6 damage
- **Natural Explorer**: 森・荒野で不利地形無視、警戒半減睡眠
- **Rogue Cunning Action**: ボーナスアクション hide/disengage/dash
- **Skirmisher**: 攻撃後10フィート後退 (free)
- **Second-Story Work**: climbing speed, jump distance double

**セービング**: DEX+7, WIS+5, CON+5, INT+4, STR+4, CHA+3

---

## 経歴

- **生誕**: 1974年、エリュシオン北西部の森
- **12歳**: 家族がモンスターの襲撃で死去、復讐を誓う
- **15歳**: 冒険者ギルドに入門、見習い開始
- **20歳**: 初のSランクモンスター単独討伐 (深淵の覇者)
- **30歳**: ギルド内で「無敵のシルヴァン」と呼ばれる
- **40歳**: 引退を表明するも、国難時に復帰
- **現在**: 50代半ば、後進の指導に専念しつつ、最後の大任務待つ

**主な戦績**:
- Sランク討伐: 12体 (世界最多記録)
- Aランク: 50体以上
- クエスト成功率: 98%
- 死亡回数: 0 (伝説の不敗)

---

## 関係者

| 関係 | 人物 | 組織 |
|------|------|------|
| **弟子** | 若手エルフ冒険者 エリアス | ギルド |
| **生涯のライバル** | オーク戦士団長 ゴア (過去に1対1で引き分け) | オーク |
| **尊敬** | 建国者レクトル | 歴史 |
| **保護** | 多くの村人から感謝される | 民間 |

---

## 所有アイテム

- **弓グリーンウッド (Longbow of Greenwood)**: +2 ロングボウ、風属性ダメージ追加、1/day 風の矢 (3d6)
- **月光のクローク**: 暗視60フィート、月明かり下で Stealth +5
- **森の護符**: 自然環境での生存判定 +5、動物との会話 (1/day)
- **影の短剣**: +1 短剣、暗所での攻撃ロールに自動クリティカル (19-20)
- **薬瓶**: ヒールポーション ×5、 antidote ×3

---

## 現在の状況

- 現在はエリュシオン中央地域で若手の育成に専念
- 年に1回、大規模モンスター退治に参加
- 引退後はギルドマスターになる予定
- 過去の傷がたびたび痛む

---

**戦闘スタイル**: 暗殺、奇襲、遠距離射撃。接近戦は避ける。森・夜間では無敵。

---

**関連項目**:
- [冒険者ギルド](../culture/calendar.md) (暦・行事で言及)
- [モンスター図鑑](../creatures/bestiary.md)
- [エルフ種族](../races/races-overview.md)
- [風の精霊ゼフ](../magic/system.md)

---

**最終更新**: 2026-05-01
**バージョン**: 1.0.0
